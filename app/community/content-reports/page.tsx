...
/**
 * /community/content-reports — moderation queue for user-submitted
 * content reports. Each row shows the reported post/comment snippet,
 * the reporter, the reason + free-text details, and quick actions:
 *   • Open original (if still alive)
 *   • Mark reviewed       → status="reviewed"
 *   • Dismiss             → status="dismissed"
 *   • Delete content      → DELETE the underlying post + status="actioned"
 *
...
export default function ContentReportsPage() {
  ...
  async function deleteContentAndAction(report: ContentReport) {
    ...
    try {
      if (report.targetType === "POST") {
        ...
        // Admin comment moderation uses PATCH action="delete"
        await authFetchJson(
      ...
      setItems((prev) => prev.map((r) => (r.id === report.id ? updated : r)));
  ...
  return (
    <div className={`p-6 min-h-screen ${isDark ? "bg-dark-bg text-dark-text" : "bg-very-light-grey text-dark-charcoal"}`}>
      <header className="mb-6 flex items-end justify-between">
        <div>
          <h1 className="text-2xl font-bold">Content Reports</h1>
          <p className="mt-1 text-sm text-[color:inherit] opacity-70">
      ...
      {loading ? (
        ...
        <div className={`rounded-2xl border ${isDark ? "border-dark-border bg-dark-card" : "border-light-grey bg-pure-white"} p-12 text-center`}>
          <p className="text-sm font-semibold">No reports yet</p>
          <p className="mt-1 text-xs opacity-70">When users flag a post or comment it will show up here.</p>
        </div>
      ) : (
          ...
          {items.map((r) => (
            ...
            >
              <div className="mb-2 flex flex-wrap items-center gap-2 text-xs">
                <span className={`rounded-full px-2 py-0.5 font-bold ${STATUS_BADGE[r.status] ?? STATUS_BADGE.pending}`}>
                  {r.status}
                </span>
              ...
              <blockquote className={`mb-2 rounded-xl border-l-2 px-3 py-2 text-sm ${
                isDark ? "border-dark-border bg-dark-bg/40" : "border-light-grey bg-very-light-grey/60"
              }`}>
              ...
              {r.details && (
                <p className="mb-3 text-xs opacity-80">
                  <span className="font-semibold">Reporter note: </span>{r.details}
                </p>
              )}
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 text-xs">
                <span className="opacity-70">
                  ...
                  {r.resolvedByName && r.resolvedAt && (
                    <> · resolved by <span className="font-semibold">{r.resolvedByName}</span> at {formatTime(r.resolvedAt)}</>
                  )}
                </span>
